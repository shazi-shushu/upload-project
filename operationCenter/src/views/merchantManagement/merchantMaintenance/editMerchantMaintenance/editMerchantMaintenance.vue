        ...
        <el-form ref="ruleForm" :model="ruleForm" status-icon :rules="rules" label-width="140px" class="demo-ruleForm">
          ...
          <div class="wapperItem">
            <el-form-item label="登录密码" prop="merchantPassword">
              <el-input v-model="ruleForm.merchantPassword" disabled type="password" />
            </el-form-item>
          </div>
          <div class="wapperItem">
...
import { updateMerchant } from '@/api/merchant/merchant'
...
export default {
  ...
  methods: {
    init() {
      this.ruleForm = Object.assign({}, this.$route.query.param)
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          updateMerchant(this.ruleForm).then((res) => {
            // console.log("add res")
            // console.log(res)
