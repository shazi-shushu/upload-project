<template>
  <div class="pictureInfor_content">
    <div class="upPictureContent">
      <span> <span class="red">*</span> 酒店图片</span>
      <span class="upImg" @click="showUpImg">图片上传</span>
    </div>
    <!-- 图片导航部分 -->
    <pic-nav :pic-nav-list="picNavList" :active-index="activeIndex" @selPicNav="selPicNav" />
    <!-- 图片公公样式部分 -->
    <pic-common v-if="activeIndex!='1'" />
    <!-- 图片展示特色部分 -->
    <pic-only v-if="activeIndex=='1'" />
    <!-- 图片上传部分 -->
    <transition>
      <up-pic v-if="upPicFlag" @closeUpImg="closeUpImg" />
    </transition>
  </div>
</template>
<script>
...
// import upPic from './picComponents/upPictureInfor'
import upPic from './picComponents/upImg'
export default {
  components: {
    picNav,
    picCommon,
