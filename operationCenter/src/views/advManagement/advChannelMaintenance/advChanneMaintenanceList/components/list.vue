<template>
  <div>
    <div class="content_tips">
      ...
      <span class="fz16 color333 addRoomTypeServer" @click="editAdv(0)">新增广告频道</span>
    </div>
    ...
    <div v-if="editFlag" class="changeAdvSpance">
      <div class="changeAdvSpanceCont">
        ...
        <div class="changeBody">
          ...
          <!-- 保存 -->
          <div class="btnCont">
            <span class="saveBtn" @click="saveAdvChanne(addNewPrams.title)">保存</span>
          </div>
        </div>
      </div>
    </div>
...
import { getAdvChanneSingle, updateAdvChanneSingle, addAdvChanneSingle, changeAdvChanneSingle } from '@/api/adv/adv'
export default {
  filters: {
  ...
  methods: {
    ...
    editAdvType(val) {
      ...
      changeAdvChanneSingle(objType).then((res) => {
    ...
    editAdv(val) {
      ...
      if (val !== 0) {
        ...
        getAdvChanneSingle(obj).then((res) => {
          // console.log('getAdvChanneSingle-res')
          // console.log(res)
          this.addNewPrams = {
            title: '编辑广告频道',
            name: res.data.data.channelName,
            content: res.data.data.channelDescribe,
    ...
    saveAdvChanne(val) {
      ...
      // 点击校验不符合规则不允许添加
      // if(this.addNewPrams.name.length>55) {
      //     this.$message({
      //       message: '广告频道名称不能大约55个字符',
      //       type: 'error',
      //       duration: 1 * 1000
      //     })
      //     return false;
      ...
      if (val === '新增广告频道') {
        const addObj = {
          'channelDescribe': this.addNewPrams.content,
          // "channelId": this.addNewPrams.channelId,
          'channelName': this.addNewPrams.name,
          'deleteFlag': this.addNewPrams.type
        }
        if (addObj.channelName === '') {
          this.$message({
            message: '广告频道名称不能为空',
            type: 'error',
            duration: 1500
        ...
        this.editFlag = false
        addAdvChanneSingle(addObj).then((res) => {
          // console.log('updateAdvChanneSingle-res')
          // console.log(res)
          if (res.data.code === 200) {
            this.$message({
        ...
        this.editFlag = false
        updateAdvChanneSingle(upObj).then((res) => {
          // console.log('updateAdvChanneSingle-res')
          // console.log(res)
          if (res.data.code === 200) {
            this.$message({
