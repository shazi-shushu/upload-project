<template>
  <div class="waperContentAdd">
    ...
    <div class="addAdvMsgItem">
      ...
      <div class="addAdvMsgItemCon">
        <el-select v-model="param.deleteFlag" style="width:40%" placeholder="请选择">
          <el-option
...
export default {
  data() {
    return {
      ...
      param: {
        ...
        advertTitle: '', // 广告标题
        channelId: '', // 广告渠道id
        deleteFlag: 0, // 0启用，1停用
        endTime: '', // 结束时间
        files: '', // 文件
        id: '', // id
        pictureUrl: '', // 图片路径
        pointId: '', // 广告位id
        redisKey: getToken(), // redisKey
        returnUrl: '', // 跳转路径
        sort: '', // 排序值
        startTime: '', // 开始时间
        title1: '', // 标题1
  ...
  methods: {
    ...
    addAdv(event) {
      ...
      paramFrom.append('files', this.param.files[0]) // 通过append向form对象添加数据
      ...
      paramFrom.append('deleteFlag', this.param.deleteFlag)
      paramFrom.append('endTime', newEndTime)
      paramFrom.append('id', this.param.id)
      paramFrom.append('pictureUrl', this.param.pictureUrl)
      paramFrom.append('pointId', this.pointValue)
      paramFrom.append('redisKey', this.param.redisKey)
      ...
      // return false
      axios.post(this.$store.state.user.url + '/syAdvert/addAdvert', paramFrom, config)
        .then(response => {
      ...
      // addAdvertdSingle(this.param).then((res) => {
      //   console.log('addAdvertdSingle')
      //   console.log(res)
      // })
    },
    ...
    handleSuccess(event, file, fileList) {
      console.log(1111312311)
      console.log(event)
      console.log(file)
      console.log(fileList)
      // 当前为假删除 需要调用删除方法从服务器删除
    ...
    getAllChanne() {
      getAllAdvertChannelName().then((res) => {
        ...
        newArr.forEach(element => {
          if (element.deleteFlag === 0) {
            this.channeArr.push(element)
          }
        })
    ...
    mustInp(val) {
      ...
      if (this.channeValue === '') {
        ...
        return false
      } else if (this.startTime === null || this.startTimeVal === null || this.startTime === '' || this.startTimeVal === '') {
        this.$message({
          message: '请选择开始时间',
          type: 'error'
        })
        return false
      } else if (this.endTime === null || this.endTimeVal === null || this.endTime === '' || this.endTimeVal === '') {
        this.$message({
          message: '请选择结束时间',
          type: 'error'
        })
