  ...
  <div class="maintenance">
      ...
      <tbody>
        <tr v-for="(item,index) in list" :key="index">
          ...
          <td class="last_list">
            ...
            <span @click="del(item.userId)">删除</span>
          </td>
        </tr>
    ...
    <div v-show="show" class="mask">
      <div class="littleMask" style="width:448px;height:374px;overflow:hidden">
        ...
        <div class="tableform">
          ...
          <li><label><span>*</span>手机号</label><input v-model="maskPhone" type="text"></li>
    ...
    <div v-show="show1" class="mask">
      <div class="littleMask">
        ...
        <div class="tableform">
          <li style="margin-top:15px"><label><span>*</span>登录名</label><input v-model="maskLoginName" type="text" maxlength="55"></li>
          <li><label><span>*</span>登录密码</label><input v-model="maskPassword" type="text"></li>
          <p id="des">密码由8-20位字母、数字组合，区分大小写</p>
          <li><label><span>*</span>用户姓名</label><input v-model="maskUserName" type="text" maxlength="55"></li>
          <li><label><span>*</span>手机号码</label><input v-model="maskPhone" type="text"></li>
          <li><label><span>*</span>所属角色</label>
            <div class="selectedItem">
              <el-select v-model="roleId" placeholder="请选择">
                <el-option
...
export default {
  ...
  methods: {
    ...
    pswReset: function(item) {
      ...
      this.maskLoginName = item.userLoginName
    },
    yyes: function() {
      var that = this
      that.show2 = false
      ...
      const obj = {
        userLoginName: this.maskLoginName,
        userId: this.userId,
        userPassword: this.initpsd
        // redisKey: getToken()
      }
    ...
    btnfc: function(item) {
      ...
      this.maskLoginName = item.userLoginName
      this.maskUserName = item.userName
      this.maskPhone = item.userNum
    ...
    save: function() {
      ...
      if (obj.userLoginName.replace(/\s*/g, '') === '') {
        this.$message({
          message: '登录名不能为空',
          type: 'error',
          duration: 1500
        ...
        return false
      } else if (obj.userLoginName.length >= 55) {
        this.$message({
          message: '登录名长度超过限制',
          type: 'error',
          duration: 1500
        ...
        return false
      } else if (obj.userName.replace(/\s*/g, '') === '') {
        this.$message({
          message: '用户姓名不能为空',
          type: 'error',
          duration: 1500
        ...
        return false
      } else if (obj.userName.length >= 55) {
        this.$message({
          message: '用户姓名字数超过限制',
          type: 'error',
          duration: 1500
        ...
        return false
      } else if (obj.userNum.replace(/\s*/g, '') === '') {
        this.$message({
          message: '手机号不能为空',
          type: 'error',
          duration: 1500
        ...
        return false
      } else if (!myreg.test(obj.userNum)) {
        this.$message({
          message: '非有效手机号',
          type: 'error',
          duration: 1500
    ...
    add: function() {
      // merchantId,roleId,userLoginName,userName,userNum,userPassword
      var obj = {
        // deleteFlag: '',
        // redisKey: getToken(),
        ...
        userName: this.maskUserName,
        userNum: this.maskPhone,
        userPassword: this.maskPassword,
        roleId: this.roleId,
        // roleName: this.maskCharacter,
      ...
      if (obj.userLoginName.replace(/\s*/g, '') === '') {
        ...
        return false
      } else if (obj.userPassword.replace(/\s*/g, '') === '') {
        this.$message({
          message: '密码不能为空',
          type: 'error',
          duration: 1500
        ...
        return false
      } else if (!reg.test(obj.userPassword)) {
        this.$message({
          message: '密码格式不正确',
          type: 'error',
          duration: 1500
