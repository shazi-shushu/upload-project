...
import { update } from '@/api/accountManage/rolePermission'
...
export default {
  ...
  created() {
    ...
    if (this.$route.query) {
      ...
      this.roleDescribe = this.$route.query.roleDescribe
      this.deleteFlag = this.$route.query.deleteFlag
      this.authIds = this.$route.query.authIds
      this.param.platform = this.$route.query.platform
      this.roleId = this.$route.query.roleId
      this.ids = this.authIds.split(',')
    }
    this.param.authIds = this.authIds
    getAuths(this.param).then(res => {
      ...
      for (let i = 0; i < msg.length; i++) {
        ...
        const obj = {
          ...
          deleteFlag: this.deleteFlag, // 传值不确定
          platform: 0
        }
        var selectArr = []
        for (var j = 0; j < msg[i].children.length; j++) {
          ...
          if (msg[i].children[j].flag) {
            selectArr.push(msg[i].children[j])
            console.log(selectArr)
          }
        }
        obj.checkedCities = selectArr
  ...
  methods: {
    ...
    handleCheckedCitiesChange(value, i) {
      ...
      // console.log('22222222', this.checkedCities)
      this.getAllChekced()
    },
